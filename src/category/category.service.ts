...
export class CategoryService {
    ...
    async getAllCategories(res: Response, userId: number, search?: string): Promise<Response> {
        try {
            ...
            return res.status(500).json({ status: 500, success: false, message: "Something went wrong while fetching categories" });
        }
    }
    ...
    async createCategory(categoryDto: CategoryDto, res: Response): Promise<Response> {
        try {
            ...
            if (existingCategory) {
                return res.status(400).json({ status: 400, success: false, message: "Category name already exists" });
            }
            ...
            return res.status(200).json({ status: 200, success: true, message: "Category created successfully", data: savedCategory });
        } catch (error) {
            console.error("Error creating category:", error.message || error);
            return res.status(500).json({ status: 500, success: false, message: "Something went wrong while creating category" });
        }
    }
    ...
    async updateCategory(categoryId: number, categoryDto: Partial<CategoryDto>, res: Response): Promise<Response> {
        try {
            ...
            return res.status(200).json({ status: 200, success: true, message: 'Category updated successfully', data: updatedCategory });
        } catch (error) {
            console.error('Error updating category:', error.message || error);
            return res.status(500).json({ status: 500, success: false, message: 'Something went wrong while updating category' });
        }
    }
}
